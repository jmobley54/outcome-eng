...
class BlogAuthorInfo extends WP_Widget {
   ...
   function widget($args, $instance) {  
	...
	?>
		...
		<?php if ( $read_more_url ) echo '<a href="' . esc_url( $read_more_url ). '">' . esc_html( $read_more_text ) . '</a>'; ?></p>
        </div>
   ...
   function form($instance) {   
		...
		<p>
		   <label for="<?php echo $this->get_field_id('bio'); ?>"><?php _e('Bio:','themnific'); ?></label>
			<textarea rows="10" name="<?php echo $this->get_field_name('bio'); ?>" class="widefat" id="<?php echo $this->get_field_id('bio'); ?>"><?php echo esc_textarea( $bio ) ; ?></textarea>
		</p>
		<p>
