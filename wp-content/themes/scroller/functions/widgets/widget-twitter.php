...
// widget function
	class tmnf_twitter_widget extends WP_Widget {
		
		public function __construct() {
			parent::__construct(
				'tmnf_twitter_widget', // Base ID
				'Themnific - Recent Tweets', // Name
				array( 'description' => __( 'Display recent tweets', 'themnific' ), ) // Args
			);
		}
		...
		//widget output
			public function widget($args, $instance) {
				...
				if ( ! empty( $title ) ){ echo $before_title . 
					...
					'<a href="https://twitter.com/'.esc_attr($instance['username']).'" target="_blank">' .esc_attr($title). '</a>'
						...
						<div class="tp_recent_tweets">
							...
							foreach($tp_twitter_plugin_tweets as $tweet){					
								if(!empty($tweet['text'])){
									...
									print '<li class="p-border"><span>'.tp_convert_links($tweet['text']).'</span><br /><a class="twitter_time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.tp_relative_time($tweet['created_at']).'</a></li>';
...
// register	widget
	function register_tmnf_twitter_widget(){
		register_widget('tmnf_twitter_widget');
	}
	add_action('widgets_init', 'register_tmnf_twitter_widget', 1)
