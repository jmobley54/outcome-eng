...
class WC_CSP_Restriction extends WC_Settings_API {
	 ...
	 * @return array
	 */
	protected function get_matching_rules_map( $restriction_data, $payload, $args = array() ) {
		...
		if ( ! empty( $restriction_data ) ) {
			...
			foreach ( $restriction_data as $i => $restriction ) {
				...
				if ( ! empty( $restriction[ $this->restricted_key ] ) && $this->check_conditions_apply( $restriction, $args ) ) {
					...
					// Generate data.
					$show_excluded = $this->show_excluded( $restriction );
					$include_data  = isset( $args[ 'include_data' ] ) ? (bool) $args[ 'include_data' ] : ( ! $show_excluded );
