	...
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $this->modifier_is( $data[ 'modifier' ], array( 'in' ) ) ) {
			...
			if ( sizeof( $product_names ) > 4 ) {
				...
				if ( sizeof( $category_names ) > 1 ) {
					$message = sprintf( _x_( 'remove %1$s from your cart', 'products in categories', 'woocommerce-conditional-shipping-and-payments' ), $products, $categories );
				} else {
	...
	public function get_admin_fields_html( $index, $condition_index, $condition_data ) {
		...
		<div class="condition_row_inner">
			...
			<div class="condition_value">
				<select name="restriction[<?php echo $index; ?>][conditions][<?php echo $condition_index; ?>][value][]" class="multiselect wc-enhanced-select" multiple="multiple" data-placeholder="<?php _e( 'Select categories&hellip;', 'woocommerce-conditional-shipping-and-payments' ); ?>">
					<?php
