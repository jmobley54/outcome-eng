...
/*
     FILE ARCHIVED ON 12:31:15 Apr 11, 2016 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 9:57:04 May 12, 2016.
...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        function fnCreateColumnSelect(oTable, aData, iColumn, nTh, sLabel, bRegex, oSelected, bMultiselect) {
			...
			if(bMultiselect) {
				...
				select.change(function () {
					...
					if ($(this).val() != "") {
						...
						oTable.DataTable().columns(iColumn).search("").draw();
						$(this).addClass("search_init");
            return;
					}
					if (bRegex) {
            ...
            var filterValue = '^' + unescape($(this).val()) + '$';
            oTable.fnFilter(filterValue, iColumn, true);
          }
					fnOnFiltered();
				});
				if (currentFilter != null && currentFilter != "")//Issue 81
