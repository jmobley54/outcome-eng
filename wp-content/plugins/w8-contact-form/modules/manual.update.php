...
if ( ! class_exists( 'manual_plugin_updater' ) ) {
	class manual_plugin_updater {
		...
		private function __construct( $pluginname, $newpluginname, $removedfiles = array(), $classname = "" ) {
			...
			{
				if ($pluginname == $key) {$this->plugindatas = $ap;break;}
			}
			if (isset($_REQUEST['manual_plugin_updater'])) $this->action = sanitize_text_field($_REQUEST['manual_plugin_updater']);
			if (isset($_REQUEST['nonce'])) $this->nonce = sanitize_text_field($_REQUEST['nonce']);
			switch ($this->action) {
				case 'upload':
			...
			if (!$this->error)
			{
				if ($this->action=="upload") {
					if (!empty($_FILES['pluginupdate_file']['name'])) $this->uploaded = $this->upload_zip($_FILES['pluginupdate_file']);
					else $this->throw_error(__("File doesn't exists or damaged.", 'manual_plugin_updater'));
					if (isset($this->uploaded['error'])) $this->throw_error($this->uploaded['error'], 'manual_plugin_updater');
					if (!$this->error)
					{
		...
		private function unzip_update($zip)
		{
			...
			$unzipfile = unzip_file( $zip['file'], $this->temp_path);
			   ...
			   $newversion_path = plugin_dir_path( __FILE__ ) . "mpu-temp/".$this->newpluginname;
					if (file_exists($newversion_path)) $this->newplugindatas = get_plugin_data( $newversion_path );
					if (!isset($this->newplugindatas['Name'])&&!$this->error) $this->throw_error(__( 'ZIP file is not compatible with this plugin or it is not a valid WordPress Plugin', 'manual_plugin_updater' ),$this->temp_path);
				    if (!$this->error) {if ($this->plugindatas['Name']!=$this->newplugindatas['Name']) $this->throw_error(__( 'Plugin name doesn\'t match. Is this the same plugin?', 'manual_plugin_updater' ),$this->temp_path);}
				   if (!$this->error)
				   {
		...
		private function throw_error($message, $deletedir = false)
		{
			$this->update_screen();
			print($message);
			if ($deletedir!=false) $this->erasedir($deletedir);
			$this->error = $message;
		}
		...
		private function update()
		{
			...
			if ($this->copy_files($this->temp_path.'/'.$newdir[0], WP_PLUGIN_DIR.'/'.$newdir[0])) $this->erasedir($this->temp_path);
			print("<p>".__( 'Removing temp folder...', 'manual_plugin_updater' )."</p>");
			if (!empty($this->removedfiles)) $this->delete_files(WP_PLUGIN_DIR.'/'.$newdir[0], $this->removedfiles);
		...
		private function delete_files($src,$targets = array())
		{
			foreach($targets as $trg)
			{
				...
				{
					if (is_dir($src . '/' . $trg)) $this->erasedir($src . '/' . $trg);
					else @unlink($src . '/' . $trg);
				}
			}
		}
			...
			if (!in_array($dir,$exception))
			{
				...
				foreach ($objects as $object) {
				  if ($object != "." && $object != "..") {
					...
					else {if (file_exists($dir."/".$object)) @unlink($dir."/".$object);}
				  }
				}
				reset($objects);
