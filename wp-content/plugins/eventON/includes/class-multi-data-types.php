...
class evo_mdt{
	...
	// Frontend
		...
		function frontend_box($object, $helpers){
			...
			echo  "<div class='evo_metarow_mdt_{$x} evo_metarow_mdt evorow evcal_evdata_row bordb evcal_evrow_sm".$helpers['end_row_class']."' data-event_id='".$object->event_id."'>
				...
				foreach($terms as $term){
					...
					echo "<p>".$tax_data[$term->term_id]['description'].'</p>';
		...
		function additional_field_values($tax, $tax_data, $termid){
			...
			for( $z=1; $z <= $this->evo_max_mdt_addfield_count(); $z++){
				if( evo_settings_check_yn($this->opt , 'evcal_mdta_'.$mdt_index) &&
					!empty( $this->opt[ 'evcal_mdta_name_'.$mdt_index. '_' .$z ]) &&
					!empty( $tax_data[$termid]['evcal_mdta_'.$mdt_index])
				){	
					echo "<p><span>" . $this->opt[ 'evcal_mdta_name_'.$mdt_index. '_' .$z] . "</span>";
					echo stripslashes($tax_data[$termid]['evcal_mdta_'.$mdt_index]) . "</p>";
				}
			}
		}	
	...
	// Event Post meta box
		...
		function content($name, $tax, $x){
			...
			<div class='evcal_data_block_style1'>
				<div class='evcal_db_data'>
					...
					<div class="evomdt_actions">
						<p><a class='evo_btn evomdt_add_new_btn ajde_popup_trig' data-popc='evomdt_new' data-tax='<?php echo $tax;?>' data-eventid='<?php echo $p_id;?>'><?php _e('Add New','eventon');?></a> 
						<a class='evo_btn evomdt_get_list ajde_popup_trig' data-popc='evomdt_list' data-eventid='<?php echo $p_id;?>' data-tax='<?php echo $tax;?>'><?php _e('Select from list','eventon');?></a></p>
					</div>
			...
			$ajde->wp_admin->lightbox_content(array(
				...
				'class'=>'evomdt_new',
				'title'=>__('Create New '.$name,'eventon'),
				'outside_click'=>false
			));
			$ajde->wp_admin->lightbox_content(array(
				'content'=>"<p class='evo_lightbox_loading'></p>",
		...
		// List to select items from
			...
			function get_mdt_term_data($tax){
				...
				if ( $terms && ! is_wp_error( $terms ) ){
					...
					$termMetas = get_option( "evo_tax_meta");
				...
				// foreach additional fields
					for( $z=1; $z <= $this->evo_max_mdt_addfield_count(); $z++){
						if( evo_settings_check_yn($this->opt , 'evcal_mdta_'.$mdt_index) &&
							...
							$base['evcal_mdta_'.$mdt_index] = array(
								$this->opt[ 'evcal_mdta_name_'.$mdt_index. '_' .$z],
								'text',
								'norequired'
							);
						}
	...
	// Supportive
		...
		function evo_max_mdt_addfield_count(){
			return apply_filters('evo_multi_data_type_fields_count',1);
		}
}