jQuery(function ($) {
    ...
    $('#wc_crm_mailchimp').change(function () {
        $('#wc_crm_mailchimp_api_key, #wc_crm_mailchimp_list').closest('tr').hide();

        if ($(this).attr('checked')) {
            $('#wc_crm_mailchimp_api_key, #wc_crm_mailchimp_list').closest('tr').show();
        }
    }).change();
});
