...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.0.0', __( 'WooCommerce Sequential Order Numbers Pro', 'woocommerce-sequential-order-numbers-pro' ), __FILE__, 'init_woocommerce_sequential_order_numbers_pro', array( 'minimum_wc_version' => '2.2', 'backwards_compatible' => '4.0.0' ) );
...
class WC_Seq_Order_Number_Pro extends SV_WC_Plugin {
	...
	/** @var boolean whether performance mode is enabled */
		...
		foreach ( $settings as $section ) {
			...
			if ( isset( $section['id'] ) && 'general_options' == $section['id'] &&
				...
				$updated_settings[] = array(
					...
					'css'      => 'min-width:300px;',
					'default'  => '',
					'desc'     => '<br />' . sprintf( __( 'Sample order number: %s', self::TEXT_DOMAIN ), '<span id="sample_order_number">' . $this->format_order_number( get_option( 'woocommerce_order_number_start' ), $this->get_order_number_prefix(), $this->get_order_number_suffix(), $this->get_order_number_length() ) . '</span>' ),
				);
				...
				$updated_settings[] = array(
					...
					'css'      => 'min-width:300px;',
					'default'  => '',
					'desc'     => '<br />' . sprintf( __( 'See the %splugin documentation%s for the full set of available patterns.', self::TEXT_DOMAIN ), '<a target="_blank" href="http://docs.woothemes.com/document/sequential-order-numbers/">', '</a>' ),
				);
	...
	/**
	 * Calculate the maximum order number, by finding the max of the existing
	 * order numbers (this can be intensive on large datasets)
	 *
	 * @since 1.7.0
	 * @param string $order_number_meta_name order number meta name,
	 *        ie _order_number or _order_number_free
	...
	private function get_has_hash_before_order_number() {
		return SV_WC_Plugin_Compatibility::is_wc_version_lt_2_3() && 'yes' == get_option( 'woocommerce_hash_before_order_number', 'yes' );
	}
 ...
 */
$GLOBALS['wc_seq_order_number_pro'] = wc_seq_order_number_pro();
