...
class WC_Shipping_UPS extends WC_Shipping_Method {
	...
	public function init_form_fields() {
		...
		$this->instance_form_fields = array(
			...
			'pickup' => array(
				...
				'type'        => 'select',
				'css'         => 'width: 250px;',
				/* translators: %1$s: Current customer classification option, %2$s default classification option */
				'description' => '' !== $classification_code ? sprintf( __( 'Disabled because Customer Classification is set to "%1$s".<br /> Pickup Type is only used when Customer Classification is "%2$s".', 'woocommerce-shipping-ups' ), $customer_classifications[ $classification_code ], $customer_classifications[''] ) : '',
				'class'       => 'chosen_select',
				'default'     => '03',
				'options'     => $this->pickup_code,
				'desc_tip'    => __( 'This will adjust the rate you get, so only change it if needed.', 'woocommerce-shipping-ups' ),
				'disabled'    => '' !== $classification_code,
			),
			'fallback' => array(
	...
	public function get_packaging_dimension( $dim, $from_unit = 'in', $to_unit = null ) {
		...
		if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '2.5', '>=' ) ) {
			return wc_get_dimension( $dim, $to_unit, $from_unit );
		}
	...
	private function get_customer_classifications() {
		return array(
