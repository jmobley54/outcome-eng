	...
	if ($u_id && $notification_type){
		...
		if (!empty($data)){
			...
			if ($subject && $message && $user_email){
				...
				if ($notification_type=='admin_user_register'){
					/////// ADMIN NOTIFICATION
...
function ihc_inside_dashboard_error_license($global=FALSE){
	...
	if (!IHCACTIVATEDMODE){
		...
		return "<div class='$class'><p>This is a Trial Version of <strong>Ultimate Membership Pro</strong> plugin. Please add your purchase code into Licence section to enable the Full Ultimate Membership Pro Version. Check your <a href='" . $url . "'>licence section</a>.</p></div>";
	}		
	return '';
}
...
function ihc_check_slug_name($str='', $remove_quotes=FALSE){
	/*
...
function ihc_get_user_id_by_user_login($u_login=''){
	...
	if (!empty($u_login)){
		global $wpdb;
		$data = $wpdb->get_row("SELECT ID FROM " . $wpdb->prefix . "users WHERE user_login='" . $u_login . "';");
		if (!empty($data->ID)){
			return $data->ID;
		}
	}
