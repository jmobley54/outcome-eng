...
function ihc_admin_run_plugin_updates(){
	...
	//==================== Register Fields
	if (get_option('ihc_update_version6')===FALSE){
...
<p>If you did not request for a new password, please ignore this Email notification.</p>';
		...
		$data = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "ihc_notifications WHERE notification_type='admin_user_register';");
		if (empty($data->id)){
			require_once IHC_PATH . 'admin/includes/functions.php';
			$notf_data = array('notification_type'=>'admin_user_register', 'level_id'=>-1);
			$notf_data['subject'] = '{blogname}: New Membership User registration';
			$notf_data['message'] = '<html><head></head><body><p>New Membership User registration on: <strong> {blogname} </strong></p>
...
<p>Have a nice day!</p>
		...
		update_option('ihc_update_version6', 1);//ihc_update_version
	}
}
