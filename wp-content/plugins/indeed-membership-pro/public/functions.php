...
function ihc_renew_level($u_id, $l_id, $payment_type=''){
	...
	switch ($payment_type){
		case 'paypal':
			$href = IHC_URL . 'public/paypal_payment.php?lid=' . $l_id . '&uid=' . $u_id;
		...
		case 'twocheckout':
			if (!empty($_REQUEST['ihc_coupon'])){
				ihc_twocheckout_submit($u_id, $l_id, $_REQUEST['ihc_coupon']);
			} else {
				ihc_twocheckout_submit($u_id, $l_id);
			}
			
			break;
		case 'authorize':
			$href = IHC_URL . 'public/authorize_payment.php?lid=' . $l_id . '&uid=' . $u_id;
			if (!empty($_REQUEST['ihc_coupon'])){
				$href .= '&ihc_coupon=' . $_REQUEST['ihc_coupon'];
			}
			wp_redirect($href);
