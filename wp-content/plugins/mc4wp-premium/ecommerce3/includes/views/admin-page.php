...
if( ! empty( $settings['last_updated'] ) ) {
	$formatted_date = date( get_option('date_format') . ' ' . get_option('time_format'), $settings['last_updated'] + ( get_option( 'gmt_offset', 0 ) * 3600 ) );
...
if( $queue ) {
	...
	echo '</div>';
		...
		<div class="margin-big well" style="background-color: rgba(255, 50, 50, 0.10);">
			<h3><?php _e( 'Reset MailChimp connection', 'mc4wp-ecommerce' ); ?></h3>
			<p>	<?php printf( __( 'Your store is currently connected to <strong>%s</strong> in MailChimp as <strong>%s</strong>.', 'mc4wp-ecommerce' ), sprintf( '<a href="%s">%s</a>', $connected_list->get_web_url(), esc_html( $connected_list->name ) ), esc_html( $settings['store']['name'] ) ); ?>
