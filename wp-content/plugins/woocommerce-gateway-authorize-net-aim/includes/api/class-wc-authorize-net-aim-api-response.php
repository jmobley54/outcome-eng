 ...
 */
class WC_Authorize_Net_AIM_API_Response extends Framework\SV_WC_API_XML_Response implements Framework\SV_WC_Payment_Gateway_API_Response, Framework\SV_WC_Payment_Gateway_API_Authorization_Response {
	 ...
	 *
	 * This will be an API error message if there was an API error, otherwise
	 ...
	 *
	 * This will be an API error code if there was an API error, otherwise the
	...
	/**
	 * Gets an error message appropriate for a frontend user.
	 *
	 * This should be used to provide enough information to a user to allow them
	 * to resolve an issue on their own, but not enough to help nefarious folks
	 * fishing for info.
	 *
