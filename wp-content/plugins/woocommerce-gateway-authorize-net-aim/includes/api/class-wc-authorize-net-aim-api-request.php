 ...
 * @since 3.0
 */
class WC_Authorize_Net_AIM_API_Request extends Framework\SV_WC_API_XML_Request implements Framework\SV_WC_Payment_Gateway_API_Request {
	...
	public function create_refund( WC_Order $order ) {
		...
		$this->request_data = array(
			'refId'              => Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ),
			'transactionRequest' => array(
	...
	private function create_transaction( $type ) {
		...
		$this->request_data = array(
			'refId'              => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'id' ),
			'transactionRequest' => array(
				'transactionType'     => $type,
				'amount'              => $this->order->payment_total,
				'currencyCode'        => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'currency', 'view' ),
				...
				'customerIP'          => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'customer_ip_address' ),
		...
		if ( 'credit_card' == $this->order->payment->type ) {
			...
			$payment = array(
				'bankAccount' => array(
					...
					'nameOnAccount' => Framework\SV_WC_Helper::str_truncate( $this->order->get_formatted_billing_full_name(), 22 ),
					'echeckType'    => 'WEB',
				),
			);
		}
	...
	private function get_line_items() {
		...
		foreach ( $this->order->get_fees() as $fee_id => $fee ) {
			...
			if ( $this->order->get_item_total( $fee ) >= 0 ) {
				...
				$line_items[] = array(
					'itemId'      => $fee_id,
					'name'        => ! empty( $fee['name'] ) ? Framework\SV_WC_Helper::str_truncate( htmlentities( $fee['name'], ENT_QUOTES, 'UTF-8', false ), 31 ) : __( 'Fee', 'woocommerce-gateway-authorize-net-aim' ),
	...
	private function get_customer() {
		...
		if ( is_email( Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_email' ) ) ) {
			$customer['email'] = Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_email' );
		}
	...
	private function get_address( $type ) {
		...
		$fields = array(
			'billing'  => array(
				'firstName'   => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_first_name' ),                                        'limit' => 50 ),
				'lastName'    => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_last_name' ),                                         'limit' => 50 ),
				'company'     => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_company' ),                                           'limit' => 50 ),
				'address'     => array( 'value' => trim( Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_address_1' ) . ' ' . Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_address_2' ) ), 'limit' => 60 ),
				'city'        => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_city' ),                                              'limit' => 40 ),
				'state'       => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_state' ),                                             'limit' => 40 ),
				'zip'         => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_postcode' ),                                          'limit' => 20 ),
				'country'     => array( 'value' => Framework\SV_WC_Helper::convert_country_code( Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_country' ) ),     'limit' => 60 ),
				'phoneNumber' => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_phone' ),                                             'limit' => 25 ),
			),
			'shipping' => array(
				'firstName' => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_first_name' ),                                         'limit' => 50 ),
				'lastName'  => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_last_name' ),                                          'limit' => 50 ),
				'company'   => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_company' ),                                            'limit' => 50 ),
				'address'   => array( 'value' => trim( Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_address_1' ) . ' ' . Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_address_2' ) ), 'limit' => 60 ),
				'city'      => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_city' ),                                               'limit' => 40 ),
				'state'     => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_state' ),                                              'limit' => 40 ),
				'zip'       => array( 'value' => Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_postcode' ),                                           'limit' => 20 ),
				'country'   => array( 'value' => Framework\SV_WC_Helper::convert_country_code( Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'shipping_country' ) ),      'limit' => 60 ),
			),
		);
		...
		foreach ( $fields[ $type ] as $field_name => $field ) {
			...
			if ( $value ) {
				$address[ $field_name ] = Framework\SV_WC_Helper::str_truncate( $value, $field['limit'] );
			}
		}
