...
class WC_Gateway_Authorize_Net_AIM_Credit_Card extends WC_Gateway_Authorize_Net_AIM {
	...
	/**
	 * Displays settings page with some additional JS for hiding conditional fields.
	 *
	...
	protected function validate_credit_card_fields( $is_valid ) {
		...
		if ( ! Framework\SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-payment-descriptor' ) ) {
		...
		if ( ! $is_valid ) {
			...
			Framework\SV_WC_Helper::wc_add_notice( $params['general_error'], 'error' );
		}
	...
	public function get_order( $order_id ) {
		...
		if ( $this->is_accept_js_enabled() && $nonce = Framework\SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-payment-nonce' ) ) {
			...
			$order->payment->opaque_descriptor = Framework\SV_WC_Helper::get_post( 'wc-' . $this->get_id_dasherized() . '-payment-descriptor' );
		}
	 ...
	 * @param \WC_Authorize_Net_AIM_API_Response|null $response API response object, if any
	 */
	...
	/**
	 * Adds Authorize.Net AIM specific data to the order for performing a refund.
	 *
	...
	protected function get_order_for_refund( $order, $amount, $reason ) {
		...
		$order->refund->account_four = $this->get_order_meta( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ), 'account_four' );

		if ( $expiry_date = $this->get_order_meta( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ), 'card_expiry_date' ) ) {
		...
		if ( ! $order->refund->account_four ) {
			...
			$error_message = __( 'Order is missing credit card last four.', 'woocommerce-gateway-authorize-net-aim' );

		} elseif ( $order->refund->amount != $order->get_total() ) {

			try {

				$response = $this->get_api()->get_transaction_details( $order->refund->trans_id, Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ) );

				if ( $response->is_captured() && ! $response->is_settled() ) {
					$error_message = __( 'The transaction cannot be partially refunded until it has settled. You can either wait for settlement to process or use the full order total to void the transaction.', 'woocommerce-gateway-authorize-net-aim' );
				} elseif ( $response->is_authorized() ) {
					$error_message = __( 'The transaction cannot be partially refunded until it has been captured and settled. Please capture the order or use the full order total to void the transaction.', 'woocommerce-gateway-authorize-net-aim' );
				}
		...
		if ( ! empty( $error_message ) ) {
			return new \WP_Error( 'wc_' . $this->get_id() . '_refund_error', sprintf( __( '%s Refund error - %s', 'woocommerce-gateway-authorize-net-aim' ), $this->get_method_title(), $error_message ) );
		}
