...
define( 'GRID_ITEMS' , '' );
define( 'MODAL_SURVEY_VERSION' , '1.9.7.7' );
define( 'MSDIRS' , '/' );
...
/*			elseif ( strpos( $args[ 'condition' ], 'categoryscore' ) !== false ) {
		...
		**/
		...
		$current_question = '-1';
			...
			if ( ! empty( $ms_session ) ) {
				$last_vote = $wpdb->get_row( $wpdb->prepare( "SELECT `qid`,`aid` FROM " . $wpdb->base_prefix . "modal_survey_participants_details WHERE `sid` = %d AND `samesession` = %d ORDER BY `qid` DESC", $sid, $ms_session ) );
				if ( $last_vote ) {
