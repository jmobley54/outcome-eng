 ...
 */
	class TF_MaxButtonBlock extends \DDElementTemplate {
		...
		public function create_element_structure() {
			...
			// element id
			$this->config['id']	   		= 'maxbutton_block';
			// element name
			$this->config['name']	 		= __('MaxButton', 'fusion-core');
			// element icon
			$this->config['icon_url']  		= MB()->get_plugin_url() . "images/mb-16.png";
		...
		public function create_visual_editor( $params ) {
			...
			if (isset($params["args"]["maxbutton_id"]))
			{
				$button_id = intval($params["args"]["maxbutton_id"]);
				$button->set($button_id);
			}
			$innerHtml  = '<div class="fusion_iconbox textblock_element textblock_element_style" id="fusion_button">';
			$innerHtml .= '<div class="bilder_icon_container"> <a title="" target="_self" class="button orange" style="selector:attrib"><span class="fusion-button-text">--</span></a> ';
			$innerHtml .= $button->display(array('echo' => false, 'load_css' => 'inline')) ;
			$innerHtml .= '</div>';
			$innerHtml .= '</div>';
		...
		function popup_elements() {
			...
			$this->config['subElements'] = array(
				array("name" => __('Shortcode ID', 'maxbuttons'),
					  'desc' => __('The Shortcode ID to include', 'maxbuttons'),
					  'id' => 'maxbutton_id',
